...
- **sw.js** - Service worker for PWA caching (cache version: v2.6.2)
...
- **Export**: Share or download all matches and player panels as a single JSON file. Prefers `navigator.share()` (the share sheet, **`files` only - never pass `title` or `text`: iOS "Save to Files" materialises any string field as its own document, saving a stray file named "Text" beside every backup**), falling back to `<a download>` - a synthetic download click is unreliable in an installed iOS PWA, which is the app's main target. Filenames are timestamped to the minute so same-day exports cannot collide. Only a **completed** export sets `lastBackupAt`; a dismissed share sheet rejects with `AbortError` and is treated as a cancellation, not a backup
- **Import**: Restore data from backup JSON files. Two phases: `analyzeImport(file)` is **pure** - it reports what an import would do without touching state - and `applyImport(analysis, resolutions)` writes. Incoming records split three ways: **new** (imported), **identical** (skipped silently), and **conflicting** (same `id`, different content). Conflicts open a resolution modal where each defaults to keeping the device copy; `resolutions` maps an id to `mine`/`theirs`, and anything unnamed keeps the device copy, so the destructive choice is never a fallback. Choosing the backup replaces the **whole** match - never an event merge, since an event edited on both sides has no correct answer. Cancelling writes nothing at all, not even the non-conflicting matches
...
- When modifying cached files, increment cache version in `sw.js` (currently v2.6.2)
